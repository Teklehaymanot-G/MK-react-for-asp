...
const ChangeStatus = props => {
  const { className, kifilManager, ...rest } = props;
  ...
  const changeStatus = () => {
    const changeStatusProps = {
      table: 'account',
      idCol: 'account_id',
      id: kifilManager.account_id,
      status: kifilManager.status === 1 ? 0 : 1,
      modifiedBy: getSessionValue('sessionAbalatId')
    }

    Axios.post(`${config.LOCATION()}changeStatus`, {changeStatusProps})
      .then(response => {
        if(response.data.success){
          kifilManager.status = kifilManager.status === 1 ? 0 : 1;
    ...
    <div>
      <IconButton 
        ...
        <StatusBullet
          ...
          color={statusColors[kifilManager.status]}
          size="sm"
        />
        
        <Typography display="inline" variant="body2">
          {statusLabel(kifilManager.status)}
        </Typography>
      </IconButton>
            ...
            <Typography component='span' style={{display: 'flex'}}>
              <Grid component='span' style={{display: 'flex', flexDirection: 'column'}}>
                ...
                <div className={classes.statusContainer}>
                  ...
                  <StatusBullet
                    className={classes.status}
                    color={statusColors[kifilManager.status === 1 ? 0 : 1]}
                    size="sm"
                  />
                  <Typography display="inline" variant="body2">
                    {statusLabel(kifilManager.status === 1 ? 0 : 1)}
                  </Typography>
                </div>
              </Grid>
            </Typography>
...
ChangeStatus.propTypes = {
  ...
  kifilManager: PropTypes.object.isRequired,
};
