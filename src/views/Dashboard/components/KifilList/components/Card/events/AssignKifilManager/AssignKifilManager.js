...
const ChangeStatus = props => {
  const { className, kifil, kifilManager, kifilAbalat, usernameList, ...rest } = props;
  const classes = useStyles();

  const username = [];
  const abalatIdOfHaveAccount = [];
  usernameList.forEach(e => {
    username.push(e.username)
    abalatIdOfHaveAccount.push(e.abalat_id)
  });
  ...
  const abalatFiltered = kifilAbalat.filter(abal => {
    if(abalatIdOfHaveAccount.indexOf(abal.abalat_id) === -1)
      return abal;
  })

  const km = kifilManager.find((m)=>{
    return m.kifil_id === kifil.kifil_id;
  })
  ...
  const assignMmMaekel = event => {
    ...
    if(km===undefined)
      var url = `${config.LOCATION()}kifil/assign`;
    else
      url = `${config.LOCATION()}kifil/update-manager/${km.account_id}`;
      ...
      <IconButton 
        ...
        onClick={handleMmModalOpen}>
          ...
          <Typography display="inline" variant="body2">
            <PersonAdd htmlColor={km === undefined ? 'red' : 'green'} />
          </Typography>
        </IconButton>
                ...
                <Typography component='span' style={{display: 'flex'}}>
                  <Grid component='span' style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                    ...
                    <form style={{width: '80%'}} onSubmit={assignMmMaekel}>
                     { km === undefined ? 'የማዕከል ኃላፊ አልተመደበም' : `አሁን ያለው የማዕከል ኃላፊ ስም - ${km.name}` }
