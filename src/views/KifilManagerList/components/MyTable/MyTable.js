...
const MyTable = props => {
  const { className, KifilManager, history, ...rest } = props;
  ...
  const handleSelectAll = event => {
    const { KifilManager } = props;

    let selectedUsers;

    if (event.target.checked) {
      selectedUsers = KifilManager.map(mm => mm.gm_id);
    } else {
      selectedUsers = [];
    }
  ...
  // const getKifilName = (searchValue) => {
        ...
        <PerfectScrollbar>
          <div className={classes.inner}>
            <Table>
              <TableHead>
                <TableRow>
                  <TableCell padding="checkbox">
                    <Checkbox
                      checked={selectedUsers.length === KifilManager.length}
                      color="primary"
                      indeterminate={
                        selectedUsers.length > 0 &&
                        selectedUsers.length < KifilManager.length
                      }
                      onChange={handleSelectAll}
                    />
                  </TableCell>
                  <TableCell>ስም</TableCell>
                  <TableCell>የክፍል ስም</TableCell>
              ...
              <TableBody>
                {KifilManager.slice(rowsPerPage * page, rowsPerPage + (rowsPerPage * page)).map(mm => (
                  <TableRow
...
MyTable.propTypes = {
  ...
  KifilManager: PropTypes.array.isRequired
};
