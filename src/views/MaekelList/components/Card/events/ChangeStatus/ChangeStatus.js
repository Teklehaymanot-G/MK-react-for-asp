...
const ChangeStatus = props => {
  const { className, maekel, refreashListFunc, ...rest } = props;
  ...
  const changeStatus = () => {
    ...
    console.log(getSessionValue('sessionAbalatId'))

    // Axios.put(`${config.LOCATION()}changeStatus`, {changeStatusProps})
    Axios.put(`${config.LOCATION()}maekel/changeStatus`, changeStatusProps)
      .then(response => {
    ...
    <div>
      <IconButton 
        ...
        <StatusBullet
          ...
          color={statusColors[maekel.status]}
          size="sm"
        />
        
        <Typography display="inline" variant="body2">
          {statusLabel(maekel.status)}
        </Typography>
      </IconButton>
            ...
            <Typography component='span' style={{display: 'flex'}}>
              <Grid component='span' style={{display: 'flex', flexDirection: 'column'}}>
                ...
                <div className={classes.statusContainer}>
                  ...
                  <StatusBullet
                    className={classes.status}
                    color={statusColors[maekel.status === 1 ? 0 : 1]}
                    size="sm"
                  />
                  <Typography display="inline" variant="body2">
                    {statusLabel(maekel.status === 1 ? 0 : 1)}
                  </Typography>
                </div>
              </Grid>
            </Typography>
