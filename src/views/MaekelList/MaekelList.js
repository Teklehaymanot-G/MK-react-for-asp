...
const MaekelList = props => {
  ...
  React.useEffect(function effectFunction() {
    if (searchValue === '') {
      ...
      axios.get(`${config.LOCATION()}maekel`).then(response => {
        setMaekel(response.data.data);
      });
    }
    // eslint-disable-next-line
  ...
  const updateList = searchValue => {
    if (searchValue === '') {
      ...
      axios
        .get(`${config.LOCATION()}maekel/search/${searchValue}`)
        .then(response => {
          setMaekel(response.data.data);
        });
    }
      ...
      <div className={classes.content}>
        <Grid container spacing={3}>
          {maekel
            ...
            .map(maekel => (
              <Grid item key={maekel.maekel_id} lg={4} md={6} xs={12}>
                <Card
                  maekel={maekel}
