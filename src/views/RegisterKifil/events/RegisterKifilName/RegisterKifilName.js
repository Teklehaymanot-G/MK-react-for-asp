...
const RegisterKifilName = props => {
  ...
  const registerKifilName = event => {
    event.preventDefault();

    Axios.post(`${config.LOCATION()}kifil/add`, {...formState.values})
      .then(response => {
  ...
  return (
    <div>
        <Typography 
          ...
          <Button onClick={handleModalOpen}><AddIcon htmlColor='#036' /> ክፍል ይመዝግቡ</Button>
        </Typography>

        <Dialog open={openKifilModal} onClose={handleModalClose} aria-labelledby='form-dialog-title' fullWidth={true}>
          ...
          <DialogContent>
            <form onSubmit={registerKifilName}>
              <DialogContentText>                
                <Typography component='span' style={{display: 'flex'}}>
                  <Grid component='span' style={{display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%'}}>
                    <div style={{width: '10%'}}></div>
                    <form style={{width: '80%'}} onSubmit={registerKifilName}>
                      {buildAlertBoxOnModal()}
                      {buildTextFields('የክፍል ስም', 'name', formState.values.name, formState.errors.name, false, [], true, 'text' )}
              ...
              <DialogActions>
                <Button disabled={!formState.isValid} type="submit">ይመዝግቡ</Button>
...
RegisterKifilName.propTypes = {
  className: PropTypes.string,
  maekel: PropTypes.object.isRequired,
};

export default RegisterKifilName;
